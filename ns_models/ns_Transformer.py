...
class Projector(nn.Module):
    ...
    '''
    def __init__(self, enc_in, seq_len, hidden_dims, hidden_layers, output_dim, kernel_size=3, extend=False):
        ...
        layers = [nn.Linear(num_input_channels * enc_in, hidden_dims[0]), nn.ReLU()]
    ...
    def forward(self, x, stats, extend=False):
        # x:     B x S x E
        # stats: B x 1 x E
        # y:     B x O
        ...
        if extend:
            ...
            zero_row = torch.zeros(x.size(0), 1, x.size(2)).to(x.device)

            x_raw_diff_padded = torch.cat([zero_row, x_diff], dim=1)

            x_diff = self.series_conv(x_raw_diff_padded)
            x = self.series_conv(x) 

            x = torch.cat([x, x_diff], dim=1)  # B x 3 x E
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.extend_mlp = configs.extend_mlp
        ...
        self.tau_learner   = Projector(enc_in=configs.enc_in, seq_len=configs.seq_len, hidden_dims=configs.p_hidden_dims, hidden_layers=configs.p_hidden_layers, output_dim=1, extend = configs.extend_mlp)
        self.delta_learner = Projector(enc_in=configs.enc_in, seq_len=configs.seq_len, hidden_dims=configs.p_hidden_dims, hidden_layers=configs.p_hidden_layers, output_dim=configs.seq_len, extend = configs.extend_mlp)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        tau = self.tau_learner(x_raw, std_enc, extend = self.extend_mlp).exp()     # B x S x E, B x 1 x E -> B x 1, positive scalar    
        delta = self.delta_learner(x_raw, mean_enc, extend = self.extend_mlp)      # B x S x E, B x 1 x E -> B x S
